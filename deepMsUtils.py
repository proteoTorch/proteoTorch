...
def load_test_scores(filenames, scoreKey = 'score', qTol = 0.01, qCurveCheck = 0.001):
    ...
    auc = np.trapz(quac)
    if qTol > qCurveCheck:
      auc = 0.3 * auc + 0.7 * np.trapz(quac[:ind0])
    return qs, ps, auc
...
def plot(scorelists, output, qrange = 0.1, labels = None, **kwargs):
  ...
  for i,(q,p) in enumerate(scorelists):
    h = max(itertools.chain([h], (b for a, b in zip(q, p) if
                                  a <= qrange)))
...
if __name__ == '__main__':
    ...
    usage = ('Usage: %prog [options] label1:IDENT1 ... labeln:IDENTn\n\n'
             'Example: %prog --output foo.png DNN:dnn-output.txt\n'
             'Example %prog --output foo.png "Deep net":dnn-output.txt')
    ...
    plot(scorelists, options.output, options.maxq,
         methods)
